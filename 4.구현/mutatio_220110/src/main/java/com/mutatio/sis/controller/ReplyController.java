...
public class ReplyController {
	...
	public Map<String, Object> qReplyList(Model model, QReplyReplyVO quesNo_pagingVO, HttpSession session){
		...
		List<QReplyVO> replyList = qReplyService.getReplyListByParent(quesNo_pagingVO); // 댓글 리스트
		boolean isToCloseMoreBtn = qReplyService.isMore(quesNo_pagingVO); // 출력 데이터가 더 있는가?
		
		Map<String, Object> map = new HashMap<String, Object>(); // return data
		map.put("result", true);
		map.put("data", replyList);
		map.put("isToCloseMoreBtn", isToCloseMoreBtn);
