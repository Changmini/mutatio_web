...
<link rel="stylesheet" type="text/css" href="../resources/css/style_recommend.css?ddㅇㅇdㅇ">
...
<div class="content">
	...
	<div class="main_div">
							...
							<td><br>
							  <label class="label_position" style="font-size:17px;"> - <strong>${sessionScope.USER_INFO.member.memName }</strong> 님의 논문조회이력
							    <i class="fas fa-book-open"></i> </label>
								    <div class="div_hisLink">
								    	 <button type="button" class="btn btn-sea ThesBtn" onclick="location.href='${pageContext.request.contextPath}/thesisList.do'"> 논문리스트 바로가기 </button>
								    </div>
								<div class="his_indiv" id="his_indiv01">
									  			<table class="innerTb" cellspacing='0'>
											    		 ...
											    		 <tbody>
											    		 <c:choose>
													    	    <c:when test="${empty referList }">
													    	        <tr>
													    	        	<td colspan="5"> 조회하신 논문이 없습니다. </td>	
													    	        </tr>
													    	    </c:when>
												    	    <c:otherwise>
							...
							<br>
								 <label class ="label_position" style="font-size:17px; text-align:left;"> - <strong>${sessionScope.USER_INFO.member.memName }</strong> 님의 자유게시판 작성글
								  <i class="fas fa-edit"></i></label>
									...
									<div class="div_hisLink">
								    	 <button type="button" class="btn btn-sea freeBtn" onclick="location.href='${pageContext.request.contextPath}/freeList.do'"> 자유게시판 바로가기 </button>
								    </div>
								<div class="his_indiv" id="his_indiv01">
										<table class="innerTb" cellspacing='0'>
									  					<colgroup>
											    	    ...
											    	    <c:otherwise>
											    		 <c:forEach items="${myFreeList }" var="free">
												    		 	<tr>
												    		 	  ...
												    		 	  <td><a href="${pageContext.request.contextPath }/freeView.do?freeNo=${free.freeNo }">${free.freeTitle }
												    		 	  	</a>
												    		 	  	</td>
												    		 	  <td>${free.freeRegDate }</td>
												    		 	  <td>${free.freeHit }</td>
												    		 	  <td class="freDel_td"><input type="hidden" value="${free.freeNo }">
								 ...
								 <label class ="label_position" style="font-size:17px; text-align:left;"> - <strong>${sessionScope.USER_INFO.member.memName }</strong> 님의 문의게시판 작성글
								  <i class="fas fa-edit"></i></label>
								   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								    ...
								    &nbsp;&nbsp;&nbsp;&nbsp; <label class="label_cnt" id="ques_cnt"> 총 ${quesRowCnt} 개의 게시물 </label>   
									<div class="div_hisLink">
								    	 <button type="button" class="btn btn-sea QuesBtn" onclick="location.href='${pageContext.request.contextPath}/member/quesList.do'"> 문의게시판 바로가기 </button>
								    </div>
								<div class="his_indiv" id="his_indiv02">
												<table class="innerTb" cellspacing='0'>
									  					<colgroup>
											    		 ...
											    		 <c:choose>
												    	    <c:when test="${empty myQuesList }">
												    	        <tr>
												    	        	<td colspan="5"> 작성하신 문의게시물이 존재하지 않습니다. </td>	
												    	        </tr>
												    	    </c:when>
												    	    <c:otherwise>
											    		 <c:forEach items="${myQuesList }" var="ques">
												    		 	<tr>
												    		 	  ...
												    		 	  <td>${ques.quesHit }</td>
												    		 	  <td class="quesDel_td"><input type="hidden" value="${ques.quesNo }">
												    		 	  <i class='fas fa-trash-alt' style='color:#2FAF53;font-size:x-large;'></i>
												    		 	  </td>
												    		 	</tr>
												    	</c:forEach>
						    ...
						    &nbsp; <i class="far fa-comment-dots" style='color:#2FAF53;font-size:x-large;'></i></label>
								...
								<div class="his_indiv" id="his_indiv01_re">
											<table class="innerTb" cellspacing='0'>
											    		 ...
											    		 <tbody>
											    		 	<c:choose>
													    	    <c:when test="${empty fReplyList }">
													    	        <tr>
													    	        	<td colspan="5"> 작성하신 자유게시판 덧글이 존재하지 않습니다. </td>	
													    	        </tr>
													    	    </c:when>
												    	    <c:otherwise>
													    		 <c:forEach items="${fReplyList }" var="fReply">
														    		 	...
														    		 	<tr class="free_reply_title">
														    		 	  <td colspan="4">
														    		 	  <a href="${pageContext.request.contextPath }/freeView.do?freeNo=${fReply.freeNo }">	
		...
		<!-- end historyTab -->
			...
			<label for="tab01">북마크&nbsp;  <i class="fas fa-star" style="font-size:x-large;"></i></label>
			<input type="radio" name="tabmenu" id="tab02" checked>
			<label for="tab02">AI 키워드&nbsp; <i class="fas fa-brain" style="font-size:x-large;"></i></label>
			<!-- <input type="radio" name="tabmenu" id="tab03">
			<label for="tab03">랭킹 Top 10&nbsp; <i class="fas fa-crown" style="font-size:x-large;"></i></label> -->
			<div class="test"></div>
			<div class="conbox con1">
			   	<table class="table-design con">
			   	 <colgroup>
					<col width="1%" />
					<col width="30%" />
					<col width="100px" />
				  	   ...
				  	   <tr>
							...
							<h3 class ="h3">인 회원들의 누적조회리스트 - </h3><br><br>
								 <strong>${sessionScope.USER_INFO.member.memName }</strong>님의 관심사는 
										...
										입니다.
								     <button type="button" class="btn btn-sea" onClick="location.href='memberInfo.do'"> 정보 수정 </button>
								<div id="tb_scroll2">
...
/* $('input[name="tabmenu"]').click(function(){
	console.log("input tabmenu 함수 실행되냐????");
	if(	$("input[id='tab01']").is(':checked')){
		$("input[id='tab01']+label").find(".fas").css('color','#fff');
		$("input[id='tab02']+label").find(".fas").css('color','#2FAF53');
		$("input[id='tab03']+label").find(".fas").css('color','#2FAF53');
	}
	if(	$("input[id='tab02']").is(':checked')){
		$("input[id='tab02']+label").find(".fas").css('color','#fff');
		$("input[id='tab01']+label").find(".fas").css('color','#2FAF53');
		$("input[id='tab03']+label").find(".fas").css('color','#2FAF53');
	}
	if(	$("input[id='tab03']").is(':checked')){
		$("input[id='tab03']+label").find(".fas").css('color','#fff');
		$("input[id='tab01']+label").find(".fas").css('color','#2FAF53');
		$("input[id='tab02']+label").find(".fas").css('color','#2FAF53');
	}
});
			...
			,success : function(resultmap){
				...
				if(resultmap.result){
					$.each(resultmap.data, function(i, elt){
					  ...
					  str += "<tr>" ;
					  str += "<td colspan='5'> 작성하신 문의게시글이 존재하지 않습니다. </td>" ;
			//		
				}//end if
					console.log("ques ajax Str ==== " + str);
					console.log("ques ajax cnt ==== " + cnt);
...
$('#menu_history').click(function(){
	...
	$('.center_div > .test').css('display', 'none');
...
function fn_history_display(){
	if(	$("input[id='histab01']").is(':checked')){
		$(".history_tab_div").css('height','730px');
...
$('.freDel_td').on("click",function(){
		 ...
		 $del_td = $(this);
		 $del_row = $del_td.closest("tr");
// recommend 단 게시글 삭제 ajax
		 console.log("freeNo == " + $freeNo);
		$.ajax({
		url:"<c:url value='/member/recommend/freeDelete'/>"
			,type : "post"
			,dataType:"text"
			,data : {"freeNo" : $freeNo}
			,beforeSend : function(xhr){
...
$('.quesDel_td').on("click",function(){
		 alert("게시글 삭제하시겠습니까?");
		 $quesNo = $(this).find('input[type=hidden]').val();
		 $del_td = $(this);
		 $del_row = $del_td.closest("tr");
// recommend 단 게시글 삭제 ajax
		 console.log("quesNo == " + $quesNo);
		$.ajax({
		url:"<c:url value='/member/recommend/quesDelete'/>"
			,type : "post"
			,dataType:"text"
			,data : {"quesNo" : $quesNo}
			,beforeSend : function(xhr){
