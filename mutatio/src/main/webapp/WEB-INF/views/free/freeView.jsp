	  ...
	  <div id="div_table">
		...
		<tbody>
			...
			<tr>
              ...
              <textarea rows="10" id="textarea_content" name="freeContent" disabled>${freeBoard.freeContent }</textarea>
              </td>
			</tr>
			<tr>
				<th>첨부파일</th>
...
<!-- // START : 댓글 목록 영역  -->		
	...
	<div class="rows text-center" id="id_reply_list_more">
...
*/
	...
	$(function(){
		$('#btn_free_edit').on("click",function(){
     		...
     		window.location.href = 'member/freeEdit.do?freeNo='+'<c:out value="${freeBoard.freeNo }" />'; 
		})
		...
		$('#btn_free_delete').on("click",function(){
     		...
     		alert("게시글 삭제하시겠습니까?");
     		window.location.href = 'member/freeDelete.do?freeNo='+'<c:out value="${freeBoard.freeNo }" />'; 
		})
	});
			...
			if(map.result){
				...
				if(elt.commentLevel == 1){	
					...
					/* 		str +="    <div class=" +"'label_div_re'" + "><button id=" + "'btn_reply_re'" + "class=" + "'btn btn-sea'" ; 
					    	...
					    	str += " <div class=" + "'edit_btns'" + "> ";
					    	str +="    <button id=" + "'btn_reply_re'" + "class=" + "'btn btn-sea'" ; 
							str +="    type="+"'button'" + ">답글</button>"; 
					    	str += " </div> ";
					    }
					    str +=" </div>";
